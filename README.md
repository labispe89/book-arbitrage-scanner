...
Standalone application in `labispe89/book-arbitrage-scanner`. It uses dependency-free ES modules, native Node tests and GitHub Pages. No automatic purchases and no scraping. Greek responsive UI, local persistence, ISBN/edition matching, editable thresholds, transparent cost calculations, import/export, optional authorized-feed backend and private scheduled alert history.

## Run

Node 20+: from repository root, `npm test`, then `npm start`. Open `http://127.0.0.1:4173/`. No install/build required. GitHub Pages deploys automatically from `main` to `https://labispe89.github.io/book-arbitrage-scanner/`. No secrets belong in frontend files or Pages.
...
No backend account or credentials are assumed. Install the official Wrangler CLI in your own environment and authenticate, then from `backend`:
